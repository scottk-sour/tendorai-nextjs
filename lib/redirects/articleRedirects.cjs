...
function parseArticles() {
  ...
  // Line endings are normalised before parsing. The block delimiters below are
  // LF-specific, and git checks this file out with CRLF on Windows (core.autocrlf
  // defaults to true there), which made every split miss and the parse return
  // zero. Vercel builds on Linux, so the failure only appeared on a local
  // Windows checkout.
  const src = fs
    .readFileSync(path.join(__dirname, '..', 'content', 'articles.ts'), 'utf8')
    .replace(/\r\n/g, '\n');
