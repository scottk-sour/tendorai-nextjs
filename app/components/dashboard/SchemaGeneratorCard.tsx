...
function buildDescriptionFallback(v: VendorData): string {
...
function generateSchemaClientSide(v: VendorData): Record<string, unknown> {
  ...
  // sameAs: vendor website + LinkedIn + regulatory register URLs only
  ...
  // hasCredential from accreditations + certifications
  const credentials = [
    ...(v.accreditations || []),
    ...(v.certifications || []),
  ...
  const schema: Record<string, unknown> = {
    ...
    areaServed: v.coverage?.map(code => ({ '@type': 'City', name: code })),
    potentialAction: {
