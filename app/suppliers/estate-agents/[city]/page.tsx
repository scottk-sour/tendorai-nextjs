...
export async function generateStaticParams() {
  return withRetry(async () => {
    ...
    const cities = await Vendor.aggregate([
      { $match: { vendorType: 'estate-agent', ...STATUS_FILTER } },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `Estate Agents in ${cityName}`;
  const description = `Compare ${vendorCount} estate agents in ${cityName}. Find verified agencies with reviews, specialisms, and AI visibility scores on TendorAI.`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://www.tendorai.com/suppliers/estate-agents/${city}`,
      type: 'website',
    },
    alternates: {
      canonical: `https://www.tendorai.com/suppliers/estate-agents/${city}`,
    },
  };
}
...
function generateFAQs(cityName: string, vendorCount: number, vendorNames: string[]) {
  const top3Names = vendorNames.slice(0, 3).join(', ') || 'top estate agents';
  const now = new Date();
  ...
  if (vendorCount >= 3) {
    return [
      {
        question: `Who are the best estate agents in ${cityName}?`,
        answer: `TendorAI lists ${vendorCount} estate agents in ${cityName}. Highly rated agencies include ${top3Names}. Propertymark members are highlighted where registered.`,
      },
      {
        question: `How many estate agents are there in ${cityName}?`,
        answer: `There are ${vendorCount} estate agents in ${cityName} listed on TendorAI as of ${currentMonthYear}.`,
      },
      {
        question: `How do I choose an estate agent in ${cityName}?`,
        answer: `Check their Propertymark membership, read client reviews, compare fee structures and services (sales, lettings, property management), and check their AI visibility score on TendorAI.`,
      },
      {
        question: `What do estate agents in ${cityName} charge?`,
        answer: `Commission rates typically range from 0.75% to 2.5% of the sale price. Letting agent fees vary by service level. Check individual firm profiles on TendorAI for current fee structures.`,
      },
      {
        question: `Does AI recommend estate agents in ${cityName}?`,
        answer: `Yes. AI assistants like ChatGPT and Perplexity already recommend specific estate agents in ${cityName} by name. Firms with verified TendorAI profiles appear more frequently in AI answers.`,
      },
    ];
  }
  ...
  return [
    {
      question: `How do I find the best estate agent in ${cityName}?`,
      answer: `TendorAI lists ${vendorCount} estate agents in ${cityName}. You can compare agencies by reviews, services, and specialisms.`,
    },
    {
      question: `Are these estate agents accredited?`,
      answer: `Where applicable, Propertymark membership is highlighted. Each profile shows the agency's accreditations and service areas.`,
    },
  ];
}

export default async function EstateAgentsInCityPage({ params }: PageProps) {
  const { city } = await params;
  const cityName = formatLocationName(city);
  ...
  const serviceAreaLinks = Object.entries(ESTATE_AGENT_SERVICE_AREA_MAP).slice(0, 6);
  ...
  const jsonLd = {
    ...
    '@graph': [
      {
        '@type': 'ItemList',
        name: `Estate Agents in ${cityName}`,
        ...
        itemListElement: allVendors.slice(0, 10).map((vendor, index) => ({
          ...
          item: {
            ...
            name: vendor.company,
            description: vendor.businessProfile?.description || 'Estate agency',
            address: {
              '@type': 'PostalAddress',
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 4, name: cityName, item: `https://www.tendorai.com/suppliers/estate-agents/${city}` },
        ],
      },
      {
        '@type': 'FAQPage',
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gray-50">
        <section className="bg-brand-gradient text-white py-12">
          <div className="section">
            <nav className="text-sm mb-4 text-purple-200">
              ...
              <span className="mx-2">/</span>
              <Link href="/suppliers/estate-agents" className="hover:text-white">Estate Agents</Link>
              <span className="mx-2">/</span>
              <span className="text-white">{cityName}</span>
            </nav>

            <h1 className="text-3xl md:text-4xl font-bold mb-4 text-white">
              Estate Agents in {cityName}
            </h1>
            <p className="text-lg text-purple-100 max-w-3xl">
              Compare {totalCount} estate agents in {cityName}. Find the right agency for selling, letting, or managing your property.
            </p>
            {totalCount >= 3 && (
              <p className="text-base text-purple-200 max-w-3xl mt-3">
                Looking for an estate agent in {cityName}? TendorAI lists {totalCount} agencies in {cityName}. Compare services, check AI visibility scores, and find the right estate agent for your needs.
              </p>
            )}
            <p className="text-sm text-purple-300 mt-2">
              Propertymark members highlighted where registered
            </p>
          </div>
        </section>
        ...
        <section className="bg-white border-b">
          <div className="section py-4">
            <p className="text-gray-600">
              <strong className="text-gray-900">{totalCount}</strong> estate agent{totalCount !== 1 ? 's' : ''} in {cityName}
            </p>
          </div>
        </section>
          ...
          <section className="bg-purple-50 border-b">
            <div className="section py-8 text-center">
              <p className="text-gray-700 text-lg mb-4">
                We&apos;re building our estate agent directory in {cityName}. Check back soon or check your AI visibility score now.
              </p>
        ...
        {totalCount >= 3 && (
          <section className="bg-white py-12 mt-8">
            <div className="section max-w-4xl">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">About Estate Agents in {cityName}</h2>
              <div className="text-gray-600 leading-relaxed space-y-4">
                <p>
                  There are {totalCount} estate agents in {cityName} on TendorAI.
                  {topPracticeAreas.length > 0 && (
                    <> The most common services in {cityName} are {topPracticeAreas.join(', ')}.</>
                  )}
                  {' '}Agencies range from independent high-street offices to regional chains.
                </p>
                <p>
                  When people ask AI assistants like ChatGPT or Perplexity for estate agents in {cityName}, AI recommends agencies with the strongest structured data. TendorAI helps {cityName} agencies get this data right.
                </p>
              </div>
              <div className="mt-6">
                <Link href="/aeo-report" className="inline-block px-6 py-3 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition-colors">
        ...
        <section className="bg-white py-12 mt-8">
          <div className="section max-w-4xl">
            <h2 className="text-2xl font-bold text-gray-900 mb-8">Frequently Asked Questions — Estate Agents in {cityName}</h2>
        ...
        <section className="bg-purple-50 py-10" data-nosnippet>
          <div className="section text-center">
            <h2 className="text-xl font-bold text-gray-900 mb-2">Estate agent in {cityName}? Claim your profile</h2>
          ...
          <div className="section">
            ...
            <div className="flex flex-wrap gap-2">
              {nearbyLocations.map((loc) => (
                <Link key={loc} href={`/suppliers/estate-agents/${loc.toLowerCase().replace(/\s+/g, '-')}`} className="px-4 py-2 bg-white rounded-full text-sm text-gray-700 hover:bg-purple-50 hover:text-purple-700 transition-colors">
                  Estate Agents in {loc}
                </Link>
              ))}
