...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `Mortgage Advisers in ${cityName}`;
  const description = `Compare ${vendorCount} FCA-authorised mortgage advisers in ${cityName}. Find verified mortgage brokers with reviews, specialisms, and AI visibility scores on TendorAI.`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://www.tendorai.com/suppliers/mortgage-advisors/${city}`,
      type: 'website',
    },
    alternates: {
      canonical: `https://www.tendorai.com/suppliers/mortgage-advisors/${city}`,
    },
  };
}
...
function generateFAQs(cityName: string, vendorCount: number, vendorNames: string[]) {
  const top3Names = vendorNames.slice(0, 3).join(', ') || 'top mortgage advisers';
  const now = new Date();
  ...
  if (vendorCount >= 3) {
    return [
      {
        question: `Who are the best mortgage advisers in ${cityName}?`,
        answer: `TendorAI lists ${vendorCount} FCA-authorised mortgage advisers in ${cityName}. Highly rated firms include ${top3Names}. All advisers are verified on the FCA register.`,
      },
      {
        question: `How many mortgage advisers are there in ${cityName}?`,
        answer: `There are ${vendorCount} FCA-authorised mortgage advisers in ${cityName} listed on TendorAI as of ${currentMonthYear}.`,
      },
      {
        question: `How do I choose a mortgage adviser in ${cityName}?`,
        answer: `Check their FCA registration status, whether they are whole-of-market, read client reviews, compare fee models, and check their AI visibility score on TendorAI.`,
      },
      {
        question: `Do mortgage advisers in ${cityName} charge a fee?`,
        answer: `Fee models vary. Some advisers are fee-free (paid by the lender), some charge a fixed fee, and some offer both options. Check individual firm profiles on TendorAI for current fee structures.`,
      },
      {
        question: `Does AI recommend mortgage advisers in ${cityName}?`,
        answer: `Yes. AI assistants like ChatGPT and Perplexity already recommend specific mortgage advisers in ${cityName} by name. Firms with verified TendorAI profiles appear more frequently in AI answers.`,
      },
    ];
  }
  ...
  return [
    {
      question: `How do I find the best mortgage adviser in ${cityName}?`,
      answer: `TendorAI lists ${vendorCount} FCA-authorised mortgage advisers in ${cityName}. You can compare firms by reviews, fee models, and specialisms.`,
    },
    {
      question: `Are these mortgage advisers regulated?`,
      answer: `Yes — every mortgage adviser listed on TendorAI is authorised and regulated by the Financial Conduct Authority. Each profile links to the firm's FCA register entry.`,
    },
  ];
}

export default async function MortgageAdvisorsInCityPage({ params }: PageProps) {
  const { city } = await params;
  const cityName = formatLocationName(city);
  ...
  const serviceAreaLinks = Object.entries(MORTGAGE_SERVICE_AREA_MAP).slice(0, 6);
  ...
  const jsonLd = {
    ...
    '@graph': [
      {
        '@type': 'ItemList',
        name: `Mortgage Advisers in ${cityName}`,
        ...
        itemListElement: allVendors.slice(0, 10).map((vendor, index) => ({
          ...
          item: {
            ...
            name: vendor.company,
            description: vendor.businessProfile?.description || 'Mortgage adviser',
            address: {
              '@type': 'PostalAddress',
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Mortgage Advisers', item: 'https://www.tendorai.com/suppliers/mortgage-advisors' },
          { '@type': 'ListItem', position: 4, name: cityName, item: `https://www.tendorai.com/suppliers/mortgage-advisors/${city}` },
        ],
      },
      {
        '@type': 'FAQPage',
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gray-50">
        <section className="bg-brand-gradient text-white py-12">
          <div className="section">
            <nav className="text-sm mb-4 text-purple-200">
              ...
              <span className="mx-2">/</span>
              <Link href="/suppliers/mortgage-advisors" className="hover:text-white">Mortgage Advisers</Link>
              <span className="mx-2">/</span>
              <span className="text-white">{cityName}</span>
            </nav>

            <h1 className="text-3xl md:text-4xl font-bold mb-4 text-white">
              Mortgage Advisers in {cityName}
            </h1>
            <p className="text-lg text-purple-100 max-w-3xl">
              Compare {totalCount} FCA-authorised mortgage advisers in {cityName}. Find the right mortgage broker for your needs.
            </p>
            {totalCount >= 3 && (
              <p className="text-base text-purple-200 max-w-3xl mt-3">
                Looking for a mortgage adviser in {cityName}? TendorAI lists {totalCount} FCA-authorised firms in {cityName}. Every adviser is verified on the Financial Conduct Authority register.
              </p>
            )}
            <p className="text-sm text-purple-300 mt-2">
              Data supplied by the{' '}
              <a href="https://register.fca.org.uk" className="underline hover:text-white" target="_blank" rel="noopener noreferrer">FCA Register</a>
            </p>
          </div>
        </section>
        ...
        <section className="bg-white border-b">
          <div className="section py-4">
            <p className="text-gray-600">
              <strong className="text-gray-900">{totalCount}</strong> mortgage adviser{totalCount !== 1 ? 's' : ''} in {cityName}
            </p>
          </div>
        </section>
        ...
        {totalCount >= 3 && (
          <section className="bg-white py-12 mt-8">
            <div className="section max-w-4xl">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">About Mortgage Advisers in {cityName}</h2>
              <div className="text-gray-600 leading-relaxed space-y-4">
                <p>
                  There are {totalCount} mortgage advisers in {cityName} on TendorAI, sourced from the FCA register.
                  {topPracticeAreas.length > 0 && (
                    <> The most common specialisms in {cityName} are {topPracticeAreas.join(', ')}.</>
                  )}
                </p>
                <p>
                  When people ask AI assistants like ChatGPT or Perplexity for mortgage advisers in {cityName}, AI recommends firms with the strongest structured data. TendorAI helps {cityName} firms get this data right.
                </p>
              </div>
              <div className="mt-6">
                <Link href="/aeo-report" className="inline-block px-6 py-3 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition-colors">
        ...
        <section className="bg-white py-12 mt-8">
          <div className="section max-w-4xl">
            <h2 className="text-2xl font-bold text-gray-900 mb-8">Frequently Asked Questions — Mortgage Advisers in {cityName}</h2>
        ...
        <section className="bg-purple-50 py-10" data-nosnippet>
          <div className="section text-center">
            <h2 className="text-xl font-bold text-gray-900 mb-2">Mortgage adviser in {cityName}? Claim your profile</h2>
          ...
          <div className="section">
            ...
            <div className="flex flex-wrap gap-2">
              {nearbyLocations.map((loc) => (
                <Link key={loc} href={`/suppliers/mortgage-advisors/${loc.toLowerCase().replace(/\s+/g, '-')}`} className="px-4 py-2 bg-white rounded-full text-sm text-gray-700 hover:bg-purple-50 hover:text-purple-700 transition-colors">
                  Mortgage Advisers in {loc}
                </Link>
              ))}
